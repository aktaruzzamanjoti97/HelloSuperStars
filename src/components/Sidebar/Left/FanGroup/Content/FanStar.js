...
const FanStar = () => {
  ...
  return (
    <>
      ...
      <div className="mt-3">
        <img src={`http://localhost:8000/${fanDetails.banner}`} alt="" className="img-fluid StartCover mb-2" />
        <b className="text-light fw-bold TitleFan">
          {fanDetails.group_name}
        </b>
        <p className="subTitleFan">
          Created at {moment(fanDetails.start_date).format('LL')} | Continue till {moment(fanDetails.end_date).format('LL')}
        </p>
      </div>
              ...
              <Nav variant="pills" className="d-flex MobileFabNave justify-content-between">
                ...
                <Nav.Item>
                  ...
                  <DropdownButton id="dropdown-basic-button" title={title} className={title === 'Shah Rukh Khan' ? `ShahrukhFan desktopFan` : title === 'Salman Khan' ? `SalmanFan desktopFan` : 'SelectFanDefault'} >
                    ...
                    <Dropdown.Item className='w-100' >
                      <p onClick={() => selectStar(my_star.first_name+' '+my_star.last_name, fanDetails.my_star)} >
                    ...
                    <Dropdown.Item >
                    <p onClick={() => selectStar(another_star.first_name+' '+another_star.last_name, fanDetails.another_star)} >
                ...
                <DropdownButton id="dropdown-basic-button" title={title} className="SelectFan mobileFan fw-bold" >
                  <Dropdown.Item href="#/action-2" style={{ backgroundColor: 'green' }}>
                    <p onClick={() => { setTitle(my_star.first_name + " " + my_star.last_name); }} >
                      {my_star.first_name} {my_star.last_name}
                    </p>
                  </Dropdown.Item>

                  <Dropdown.Item >
                    <p onClick={() => { setTitle(another_star.first_name + " " + another_star.last_name); }} >
                      {another_star.first_name} {another_star.last_name}
                    </p>
                  </Dropdown.Item>
                </DropdownButton>
